...
export const validatePhoneNumber = (phone: string): { valid: boolean; message?: string } => {
  ...
  if (!phoneRegex.test(phone)) {
    return { valid: false, message: 'Format nomor telepon tidak valid (contoh: 081234567890)' };
  }
...
export const validateDate = (date: string, fieldName: string): { valid: boolean; message?: string } => {
  if (!date) {
    return { valid: false, message: `${fieldName} tidak boleh kosong` };
  }
  ...
  if (!dateRegex.test(date)) {
    return { valid: false, message: `Format ${fieldName} tidak valid (YYYY-MM-DD)` };
  }
...
export const validateFileSize = (base64: string, maxSizeMB: number): { valid: boolean; message?: string } => {
  if (!base64) {
  ...
  if (sizeInMB > maxSizeMB) {
    return { valid: false, message: `Ukuran file terlalu besar (maksimal ${maxSizeMB}MB)` };
  }
  
  return { valid: true };
};

export const validateFileType = (base64: string, allowedTypes: string[], fieldName: string): { valid: boolean; message?: string } => {
  if (!base64) {
    return { valid: false, message: `${fieldName} tidak boleh kosong` };
  }
  ...
  if (!allowedTypes.includes(mime)) {
    return { 
      valid: false, 
      message: `Format ${fieldName} tidak didukung. Format yang didukung: ${allowedTypes.map(type => type.split('/')[1]).join(', ')}` 
    };
  }
...
export const validateTextLength = (text: string, fieldName: string, minLength: number, maxLength?: number): { valid: boolean; message?: string } => {
  if (!text) {
    return { valid: false, message: `${fieldName} tidak boleh kosong` };
  }
...
export const validateSelect = (value: string, fieldName: string): { valid: boolean; message?: string } => {
  if (!value) {
...
export const validateForm = (
  ...
  validations: Record<string, (value: unknown) => { valid: boolean; message?: string }>
